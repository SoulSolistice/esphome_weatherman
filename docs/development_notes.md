...
still open). We kept `entity_category` because it gives correct sectioning in
...
**Repo owner — resolved.** The real repository is
`https://github.com/SoulSolistice/esphome_weatherman`, so `SoulSolistice` is now
canonical everywhere: `weatherman_repo`, `project.name`, `CODEOWNERS`, and the
package-header comments (which had briefly read `SoulSoliste`) are all uniform.
Still pin `weatherman_ref` to a tag for reproducible release builds — it defaults
to `main` for first-run convenience, which is not reproducible.
...
**BMP280, not BME280.** The carrier boards are visually identical; the chip
differs (no humidity register on the BMP280). A `bme280_i2c` + humidity entity
silently never publishes on this hardware. Platform is `bmp280_i2c`; revert
instructions are in a comment for anyone with the BME280 variant.
...
**Rain comb exposes two metrics pending a drip test.** Both an edge-rate (Hz)
and a duty-cycle (%) metric are published until the drip test
(`docs/hardware-rain-sensor.md`) determines which carries the wetness signal.
The rain detector currently uses the duty-cycle metric; switching to Hz means
changing both the lambda input and the threshold scale.
...
The rule: rain *suppresses* the anti-dew duty (drying the comb would suppress
the very signal we want), but rain does **not** suppress frost protection
(freezing precipitation must still get the comb heated). The applied duty is
`max(dew, frost)`. A NaN outdoor temperature commands the heater explicitly off
(never latches at the last value).
